...
class Semantic_Mapping(nn.Module):
    ...
    def __init__(self, args, config, device):
        ...
        self.dropout = 0.5
        self.fov = 2 * np.arctan(np.tan(config["vertical_fov"] / 180.0 * np.pi / 2.0) * config["image_width"] / config["image_height"]) / np.pi * 180.0
        ...
        self.agent_height = config["camera_height"] * 100.
        self.shift_loc = [self.vision_range *
                          self.resolution // 2, 0, np.pi / 2.0]
        self.camera_matrix = du.get_camera_matrix(
            self.screen_w, self.screen_h, self.fov)
        self.camera_elevation_degree = config["camera_elevation_degree"]
